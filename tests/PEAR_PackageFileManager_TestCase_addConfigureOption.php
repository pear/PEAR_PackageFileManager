...
class PEAR_PackageFileManager_TestCase_addConfigureOption extends PHPUnit_TestCase
{
    /**
    ...
    function PEAR_PackageFileManager_TestCase_addConfigureOption($name)
    {
    ...
    {
        if (!$this->_methodExists('addConfigureOption')) {
            return;
        }
        ...
        $this->packagexml->addConfigureOption('frog', 'Sound a frog makes', 'ribbit');
        $this->assertEquals('true', $this->errorThrown, 'error not thrown');
    }

    function test_valid_add1()
    {
        ...
        $this->assertEquals(
            ...
            $this->packagexml->_packageXml['configure_options'][0],
            'configure_options value wrong');
    }

    function test_valid_add2()
    {
        if (!$this->_methodExists('setOptions')) {
            return;
        }
        ...
        $this->assertFalse($this->errorThrown, 'error thrown');
        $this->packagexml->_packageXml['maintainers'] = array();
        $this->packagexml->addConfigureOption('frog', 'Sound a frog makes');
        $this->assertFalse($this->errorThrown, 'error thrown');
        $this->assertEquals(
    ...
    function test_valid_replace()
    {
        ...
        $this->assertEquals(
            ...
            $this->packagexml->_packageXml['configure_options'],
            'configure_options value wrong');
        $this->packagexml->addConfigureOption('frog', 'Sound a frog makes');
        $this->assertFalse($this->errorThrown, 'error thrown');
        $this->assertEquals(
